...
function HTMLManager(data, history){
	...
	_this.resize = function(){
		...
		// Video height
		$("#project_page #video").css({height: $("#project_page #video").width() * 0.75});
	...
	_this.setInfo = function(code){
		...
		for(var i = 0; i < dataInfo.urls.length; i ++){
			console.log("dataInfo.urls", dataInfo.urls);
	...
	function generateMarkupForSelections(){
		...
		// ALL PROJECTS
		parent.append('<div class="container" id="list_all"><div class="title  font-effect-outline">OTHER PROJECTS</div></div>');
		...
		$("#navBottom .resume").on("click", function(){
			window.open("media/docs/Jeremy Tani 2016.pdf", "_blank");
		})
