...
function DataManager(url){
	...
	_this.createCode = function(str){
		var code = String(str).toLowerCase().replace(/ /g,'').replace("’",'').replace("'",'').replace(":",'').replace("&",'').replace(/\//g,'');
	...
	function loadDataComplete(xml){
	    ...
	    for(var i = 0; i < items.length; i ++){
	        var item = {
	            ...
	            videoWidth: Number($(items[i]).find("videoWidth").text() ),
	            videoHeight: Number($(items[i]).find("videoHeight").text() ),
	            code: _this.createCode($(items[i]).find("title").text())
	        }
